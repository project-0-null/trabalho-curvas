...
1.  **Problema 1 (Circuito RC):** Ajuste de modelo exponencial ($i = ae^{bt}$) para dados de corrente x tempo[cite: 74, 78].
2.  **Problema 2 (Volume de Árvores):** Ajuste de modelo de potência ($v = ax^b$) para dados de volume x diâmetro[cite: 85, 92].
